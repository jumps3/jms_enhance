...
class EnhanceService(BaseService):
    ...
    def _get_protocols(self, protocolstr, platform):
        ...
        if not protocolstr:
            ...
            for s in ss:
                pt = s.split("/")
                protocols.append({'name': pt[0].strip(), 'port': int(pt[1].strip())})
        return protocols

    def _add_asset_host(self, platform, row: pd.Series):
        node_ids = self._check_nodes(row['节点路径'])
        account = self._get_account(self._pure(row['账号']), self._pure(row['私钥']), self._pure(row['密码']))
        accounts = [account] if account else []
        protocols = self._get_protocols(self._pure(row['协议组']), platform)
    ...
    @staticmethod
    def _get_user_phone(phone):
        if isinstance(phone, float):
            phone = str(int(phone))
        return '+86%s' % phone if phone else ''
    ...
    def _add_user(self, row: pd.Series):
        ...
        try:
            ...
            for oname in org_names:
                ...
                root_user = self._exist_user(self._pure(row['*用户名']))

                self._set_org(oname)
                # 平台中存在用户
                if root_user:
                    user = self._exist_user(self._pure(row['*用户名']))
                    if not user:
                        # 组织中无此用户，邀请
                ...
                # 失效日期为空时自动设置默认 70 年后过期
                expired_date = self._pure(row['失效日期'])
                if expired_date == '':
                    now = datetime.datetime.now(tz=pytz.timezone('Asia/Shanghai'))
                    ed = now.replace(year=now.year+70)
                    expired_date = ed.strftime('%Y/%m/%d %H:%M:%S %z')
                ...
                data = {
                    ...
                    "phone":  self._get_user_phone(self._pure(row['手机'])),
    ...
    def _get_entities(self, entity_str, entity_type):
        ids = []
        unexists = []
        if entity_str:
            names = self._split_commas(entity_str)
            for name in names:
                name = name.strip()
                params = {'name': name}
                if entity_type == 'users':
                    results = self.jms_client.user.list(**params)
                elif entity_type == 'assets':
                    results = self.jms_client.asset.list(**params)
                elif entity_type == 'nodes':
                    params = {'search': name}
                    nodes = self.jms_client.node.list(**params)
                    results = [n for n in nodes if n.full_value == name]
                else:
    ...
    def _get_accounts(self, row: pd.Series):
        ...
        for ia in input_accounts.keys():
            ...
            if isinstance(value, bool):
                ...
                accounts.append(ia)
                if value:
                    names = self._split_commas(value.strip())
    ...
    def _add_perm(self, row: pd.Series):
        ...
        user_ids, unexists_users = self._get_entities(self._pure(row['用户']), 'users')
        ...
        asset_ids, unexists_assets = self._get_entities(self._pure(row['资产']), 'assets')
        if len(unexists_assets) > 0:
            return FAILED, '存在资产未找到，请检查资产名：%s' % unexists_assets

        node_ids, unexists_nodes = self._get_entities(self._pure(row['节点']), 'nodes')
        if len(unexists_assets) > 0:
    ...
    '''
    def import_hosts(self, data: io.BytesIO, file_type='xlsx'):
        try:
